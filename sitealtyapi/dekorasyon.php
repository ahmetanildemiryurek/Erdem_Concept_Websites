...
<body>
        ...
        <div class="container" style="padding-top: 100px; font-weight: bold; text-transform: uppercase;" >
          <div>
          <?php 
            $projeler = $db->query('SELECT * FROM projeler WHERE mimari=1', PDO::FETCH_ASSOC);
            if($projeler->rowCount()):
              foreach($projeler as $proje):
          ?>
          <h4><?=$proje['projeadi']?></h4>

          <div ID="ngy2p-<?=$proje['id']?>"  data-nanogallery2='{
              "itemsBaseURL": "../adminpaneli/projeImg/",
              "thumbnailWidth": "100",
              "thumbnailLabel": {
            ...
            <?php
              $projeImages = json_decode($proje['resim']);
              foreach($projeImages as $img):
            ?>
